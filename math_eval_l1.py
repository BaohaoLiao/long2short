...
PROMPT_TEMPLATES = {
    "l1-max": (
        ...
        " Think for maximum {num_tokens} tokens.<｜Assistant｜><think>\n"
    ),
    "l1-max-choice": (   # For multiple choice question
        "<｜begin▁of▁sentence｜>"
        "<｜User｜>{input}\n\n"
        "Answer the above multiple choice question. "
        "The last line of your response should be of the following format: "
        "'ANSWER: $LETTER' (without quotes) where LETTER is one of ABCD. "
        "Let's think step by step. Think for maximum {num_tokens} tokens.<｜Assistant｜><think>\n"
    ),
}
...
def main(args, llm, tokenizer, data_name):
    ...
    for i, example in tqdm(enumerate(examples), total=len(examples)):
        ...
        if example["question"] == "":
            continue
        full_prompt = PROMPT_TEMPLATES[args.prompt_type].format(input=example["question"], num_tokens=args.max_tokens_per_call)
